...
pat = 'YOUR_PAT_HERE'
excel_file_path = 'FILE_PATH_HERE'
count = 0
errorCount = 0
...
email_column_name = 'Username'  # Change this to the name of the column containing the email IDs
current_access_column_name = 'Access Level'
change_access_to_column_name = 'Changes To Be'
...
for index, row in emails_df.iterrows():
    ...
    if not new_access_level:
        errorCount += 1
        print(f'--Error{errorCount}--: Invalid access level: {user_change_to_access_level} for user {user_email}')
        continue
